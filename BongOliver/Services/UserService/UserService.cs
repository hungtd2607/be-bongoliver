...
namespace BongOliver.Services.UserService
{
    public class UserService : IUserService
    {
        ...
        public ResponseDTO CreateUser(CreateUserDTO createUserDTO)
        {
            ...
            if (user != null) return new ResponseDTO { Code = AppConst.FAILED_CODE, Message = "User đã tồn tại!" };

            var role = _roleRepository.GetRoleById(createUserDTO.RoleId);
        ...
        public ResponseDTO DeleteUser(int id)
        {
            ...
            if (user.IsDelete) return new ResponseDTO { Code = AppConst.FAILED_CODE, Message = "User đã bị vô hiệu hóa!" };

            user.IsDelete = false;

            _userRepository.UpdateUser(user);
            if (_userRepository.IsSaveChanges()) return new ResponseDTO() { Message = "Vô hiệu hóa thành công!" };
            else return new ResponseDTO() { Code = AppConst.FAILED_CODE, Message = "Vô hiệu hóa thất bại!" };
        }
        ...
        public ResponseDTO GetUsers(int? page = 1, int? pageSize = 10, string? key = "", string? sortBy = "ID")
        {
            ...
            if (total == 0) return new ResponseDTO() { Code = AppConst.FAILED_CODE, Message = "Không có User nào" };
